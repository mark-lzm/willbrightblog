...
tags:
  - Java并发
abbrlink: a86f38b7
date: 2022-11-16 21:07:22
---


# 进程、线程、同步

参考资料：[JavaNote/Prog.md at main · Seazean/JavaNote · GitHub](https://github.com/Seazean/JavaNote/blob/main/Prog.md)
...
![image-20220702182349201](.\image-20220702182349201.png)
...
当 Context Switch 发生时，需要由操作系统保存当前线程的状态（PCR 中），并恢复另一个线程的状态，包括程序计数器、虚拟机栈中每个栈帧的信息，如局部变量、操作数栈、返回地址等
...
进程的状态参考**操作系统**：创建态、就绪态、运行态、阻塞态、终止态

![image-20220702223924241](.\image-20220702223924241.png)
...
![image-20220702195852259](.\image-20220702195852259.png)
...
- RUNNABLE <--> TIMED_WAITING：调用 obj.wait(long n) 方法、当前线程调用 t.join(long n) 方法、当前线程调用 Thread.sleep(long n)、当前线程调用LockSupport.parkNanos(long nanos)或LockSupport.parkUntil(long millis)
...
- 在静态方法中使用 类名.class 字节码作为锁对象，因为静态成员属于类，被所有实例对象共享，所以需要锁住类
...
- Mark Word 结构：最后两位是**锁标志位**
  ...
  ![image-20220703114805063](.\image-20220703114805063.png)

- 64 位虚拟机 Mark Word：

  ![image-20220703121151103](.\image-20220703121151103.png)
...
- 当 Thread-2 执行 synchronized(obj) 就会将 Monitor 的所有者 Owner 置为 Thread-2，Monitor 中只能有一个 Owner，**obj 对象的 Mark Word 指向 Monitor**，把**对象原有的 MarkWord 存入线程栈中的锁记录**中（轻量级锁部分详解）
  ...
  ![image-20220703121619863](.\image-20220703121619863.png)
...
![image-20220703154123828](.\image-20220703154123828.png)
...
![image-20220703161223823](.\image-20220703161223823.png)
...
![image-20220703161324722](.\image-20220703161324722.png)
...
- 创建锁记录（Lock Record）对象，每个线程的**栈帧**都会包含一个锁记录的结构，存储锁定对象的 Mark Word
  ...
  ![image-20220703161641313](.\image-20220703161641313.png)

- 让锁记录中 Object reference 指向锁住的对象，并尝试用 CAS 替换 Object 的 Mark Word，将 Mark Word 的值存入锁记录

  ​	![image-20220703162131959](.\image-20220703162131959.png)

- 如果 CAS 替换成功，对象头中存储了锁记录地址和状态 00（轻量级锁），表示由该线程给对象加锁

  ​	![image-20220703162427998](.\image-20220703162427998.png)
...
- 如果 CAS 失败，有两种情况：
  ...
  - 如果是线程自己执行了 synchronized 锁重入，就添加一条 Lock Record 作为重入的计数
    ...
    ​	![image-20220703162719703](.\image-20220703162719703.png)
...
- 当 Thread-1 进行轻量级加锁时，Thread-0 已经对该对象加了轻量级锁
  ...
  ![image-20220703163415138](.\image-20220703163415138.png)

- Thread-1 加轻量级锁失败，进入锁膨胀流程：为 Object 对象申请 Monitor 锁，**通过 Object 对象头获取到持锁线程**，将 Monitor 的 Owner 置为 Thread-0，将 Object 的对象头指向重量级锁地址，然后自己进入 Monitor 的 EntryList BLOCKED

  ![image-20220703163438786](.\image-20220703163438786.png)

- 当 Thread-0 退出同步块解锁时，使用 CAS 将 Mark Word 的值恢复给对象头失败，这时进入重量级解锁流程，即按照 Monitor 地址找到 Monitor 对象，设置 Owner 为 null，monitor把 "lock record 地址 00" 记录还给Thread-0，Thread-0把 "Hashcode Age Bias 01" 传给Monitor，这样Thread-0便成功解锁，唤醒 EntryList 中 BLOCKED 线程
...
- 自旋成功的情况：
  ...
  ![image-20220703165500851](.\image-20220703165500851.png)

- 自选失败的情况：

  ![image-20220703170344265](.\image-20220703170344265.png)
...
![image-20220703213712956](.\image-20220703213712956.png)
...
![image-20220703221336479](.\image-20220703221336479.png)
...
![image-20220703221416667](.\image-20220703221416667.png)
...
![image-20220704113416013](.\image-20220704113416013.png)
...
![image-20220704113416013](.\image-20220704113416013.png)
![image-20220704115630979](.\image-20220704115630979.png)
...
![image-20220704170026511](.\image-20220704170026511.png)
